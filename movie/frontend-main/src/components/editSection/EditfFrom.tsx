  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(
        `https://moviesbackend-3rb8.onrender.com/movie/${item.id}`,
        updatedMovie,
        {
          headers: {
            accept: "*/*",
      ...
      <div className="flex justify-between relative gap-[9rem] rounded-[10px] mt-[60px] ">
        ...
        <div className="right-0 place-content-start">
          ...
          <div className="mt-20 flex gap-[5px]">
            <button
              className="w-[160px] h-[50px]  py-[16px] px-[55px] rounded-[10px] border-[1px] gap-[5px]"
              onClick={handleCancel}
            >
              Cancel
            </button>
            <button
              className="primary w-[170px] h-[50px]  rounded-[10px]  gap-[5px]"
